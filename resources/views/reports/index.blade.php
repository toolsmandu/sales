...
@section('content')
    <div class="dashboard-grid">
        ...
        <section class="dashboard-content stack">
            @if ($mode === 'sales-statement')
                <section id="sales-statement" class="card stack">
                    <header class="reports-header">
                        ...
                        <form method="GET" action="{{ route('reports.sales-statement') }}" style="display: flex; align-items: flex-end; gap: 0.5rem; flex-wrap: wrap; justify-content: flex-end; margin-left: auto; width: 100%; max-width: 520px;">
            ...
            @if ($mode === 'main' || $mode === 'top-selling')
            <section id="top-selling-reports" class="card stack">
                <header class="reports-header">
                    <div>
                ...
                <div class="reports-table-toolbar">
                    <form class="reports-filters" method="GET" action="{{ $mode === 'top-selling' ? route('reports.top-selling') : route('reports') }}">
                        <label>
                            <span>Month</span>
                            <select name="month">
                                @foreach ($months as $month)
            ...
            @if ($mode === 'main' || $mode === 'customer-list')
                <div id="customer-list" class="reports-subsection">
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap;">
                        <div>
                            <h2 style="margin: 0;">Customer List</h2>
                        </div>
                        <form method="GET" action="{{ $mode === 'customer-list' ? route('reports.customer-list') : route('reports') }}" class="reports-contact-filters">
                            ...
                            <div class="reports-contact-actions">
                                ...
                                <a
                                    ...
                                    href="{{ $mode === 'customer-list' ? route('reports.customer-list', array_merge(request()->all(), ['export' => 'contacts'])) : route('reports', array_merge(request()->all(), ['export' => 'contacts'])) }}"
